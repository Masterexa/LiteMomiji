...
size_t getAlignedSize(size_t src, size_t align)
{
	auto mod = src%align;
	return (mod!=0 ? (align-mod) : 0)+src;
}


HRESULT Model::init(Graphics* graphics, ModelDesc* p_desc)
{
	HRESULT	hr;
	auto	p_device		= graphics->m_device.Get();
	size_t	index_offset	= getAlignedSize(sizeof(Vertex)*p_desc->verts_count, 64);

	// create the heap for buffers
	{
		D3D12_HEAP_DESC	desc={};
		desc.SizeInBytes	= index_offset + getAlignedSize(sizeof(uint32_t)*p_desc->verts_count, 64);
		desc.Alignment		= 0;
